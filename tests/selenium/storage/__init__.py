...
def check_vulns_multiactions(sclnt, dt_id):
    ...
    dt_elem.find_element(By.XPATH, '(//tr[@role="row"]/td[contains(@class, "select-checkbox")])[1]').click()
    toolbar_elem.find_element(By.XPATH, '//a[contains(@class, "abutton_tag_multiid") and text()="Info"]').click()
    dt_elem = dt_wait_processing(sclnt, dt_id)
    ...
    toolbar_elem.find_element(By.XPATH, '//a[contains(@class, "abutton_tag_multiid") and text()="Report"]').click()
    dt_elem = dt_wait_processing(sclnt, dt_id)
